...
By default the demo is simulated. The backend it implies is **fully designed** ([`docs/BACKEND.md`](docs/BACKEND.md), [`docs/schema.sql`](docs/schema.sql), [`docs/API.md`](docs/API.md)) and its **critical path is now built AND wired into the app**: drop in a `config.js` with your Supabase keys and *sign in → book this Thursday → group chat → check in* run on real, server-authoritative Postgres (auth, the weekly-cap join funnel, reservations, realtime chat, attendance) — with no `config.js`, the same `index.html` stays the pure simulation. To stand it up: **[`docs/DEPLOY.md`](docs/DEPLOY.md)** (≈10 min, free tier). Legal text is a template pending attorney review (mapped in [`docs/LEGAL.md`](docs/LEGAL.md)). **No budget? Start with [`docs/BOOTSTRAP.md`](docs/BOOTSTRAP.md).**
